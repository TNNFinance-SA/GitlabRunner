...
RUN apt-get --allow-releaseinfo-change update && apt-get install -y curl git python python-pip jq libssl-dev curl ruby rubygems curl puppet apt-transport-https ruby-dev sudo  unzip wget  rsync software-properties-common libjpeg62-turbo-dev ssh apt-transport-https ca-certificates curl gnupg-agent software-properties-common
